...
// Project data with coordinates and descriptions
const projects = [
    {
        id: 1,
        name: "Ermera Market",
        description: "Description for Project 1",
        coordinates: [125.431240625443479, -8.719496465892805], // Longitude, Latitude
    },
    {
        id: 2,
        name: "Lautem Com",
        //description: "Description for Project 2",
        coordinates: [126.903449027101303,-8.363299607225683], // Longitude, Latitude
    },
    {
        id: 3,
        name: "Lautem Lospalos",
        description: "Description for Project 3",
        coordinates: [126.995739891430915,-8.522167198889415], // Longitude, Latitude
    },
	{
        id: 4,
        name: "Same Betano",
        description: "Description for Project 1",
        coordinates: [125.694683261555596,-9.100052595162238], // Longitude, Latitude
    },
    {
        id: 5,
        name: "Manehat Nartabora Bridge",
        description: "Description for Project 2",
        coordinates: [126.007236982842201,-8.957736755265186], // Longitude, Latitude
    },
    {
        id: 6,
        name: "Baucau Market",
        description: "Description for Project 3",
        coordinates: [126.441349443737721,-8.479672321186122], // Longitude, Latitude
    },
	{
        id: 7,
        name: "Letefoho Gleno",
        description: "Description for Project 3",
        coordinates: [125.42736,-8.83574], // Longitude, Latitude
    },

];

// Create an array to store the overlays
const overlays = [];

// Add pop-ups for each project
projects.forEach((project) => {
    // Create a custom popupz element
    const popupElement = document.createElement("div");
    popupElement.style.position = "absolute";
    popupElement.style.background = "rgba(255, 255, 255, 0.2)"; // Transparent white background
    popupElement.style.backdropFilter = "blur(10px)"; // Blur effect
    popupElement.style.webkitBackdropFilter = "blur(10px)"; // Blur effect for Webkit browsers
    popupElement.style.padding = "10px"; // Increased padding for better spacing
    popupElement.style.border = "1px solid rgba(255, 255, 255, 0.3)"; // Soft white border
    popupElement.style.borderRadius = "10px"; // Rounded corners
    popupElement.style.boxShadow = "0 4px 6px rgba(0, 0, 0, 0.1)"; // Subtle shadow
    popupElement.style.textAlign = "center";
    popupElement.style.minWidth = "175px"; // Slightly wider for better layout
	popupElement.style.color = "white"; // Set font color to white

    // Set the content of the pop-up
    popupElement.innerHTML = `
        <h4>${project.name}</h4>

        <button onclick="zoomTo(${project.coordinates[0]}, ${project.coordinates[1]}, ${project.id})">Zoom to Location</button>
    `;

    // Create an overlay for the popupz
    const popupOverlay = new ol.Overlay({
        element: popupElement,
        position: ol.proj.fromLonLat(project.coordinates),
        positioning: "bottom-center",
        stopEvent: true,
    });

    // Add the overlay to the map and store it in the overlays array
    map.addOverlay(popupOverlay);
    overlays.push(popupOverlay);
});

// Function to zoom to a specific location and add "Back" button
window.zoomTo = function (lon, lat, projectId) {
    ...
    // Update the pop-up content with a "Back" button
    const popupElement = overlays[projectId - 1].getElement();
    popupElement.innerHTML = `
        <h4>${projects[projectId - 1].name}</h4>
        <button 
            onclick="zoomBack()" 
            style="
				all: unset;
            ...
            " 
            onmouseover="this.style.background='linear-gradient(135deg, #2986cc, #6fa8dc)'" 
            onmouseout="this.style.background='linear-gradient(135deg, #4caf50, #81c784)'"
        >
            Back
        </button>
    `;
...
// Function to zoom back to the original extent
window.zoomBack = function () {
    // Set the view back to the initial extent or desired zoom
    map.getView().animate({
        center: ol.proj.fromLonLat([126.431240625443479, -8.719496465892805]), // Replace with your desired center
        zoom: 9.5, // Replace with your desired zoom level
        duration: 1000,
    });

    // Optionally, reset all pop-ups or update the state
...
var layerSwitcher = new ol.control.LayerSwitcher({tipLabel: "Layers",  activationMode: 'click', collapsible: false});
map.addControl(layerSwitcher);

map.getView().fit([13979233.432281, -1051315.818884, 14160757.273465, -913423.203840], map.getSize());
